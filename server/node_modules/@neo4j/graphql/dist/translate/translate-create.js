    ...
    if (nodeProjection) {
        const projectionFromInput = varNameVariables.map((varName) => {
            ...
            if (projection.meta?.authValidatePredicates?.length) {
                const projAuth = cypher_builder_1.default.apoc.util.validate(cypher_builder_1.default.not(cypher_builder_1.default.and(...projection.meta.authValidatePredicates)), constants_1.AUTH_FORBIDDEN_ERROR, new cypher_builder_1.default.Literal([0]));
                return {
        ...
        parsedProjection = projectionFromInput.reduce((acc, { projection, projectionSubqueries, projectionAuth }) => {
            return {
                authCalls: cypher_builder_1.default.concat(acc.authCalls, projectionAuth),
