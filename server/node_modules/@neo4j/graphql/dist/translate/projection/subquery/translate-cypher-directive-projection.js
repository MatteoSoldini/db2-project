...
function translateCypherDirectiveProjection({ context, cypherField, field, node, alias, nodeRef, res, cypherFieldAliasMap, }) {
    ...
    if (referenceNode) {
        ...
        referencedNodes.forEach((refNode) => {
            if (refNode) {
                ...
                if (fieldFields[refNode.name]) {
                    ...
                    const projection = new cypher_builder_1.default.RawCypher((env) => `{ __resolveType: "${refNode.name}", ${str.getCypher(env).replace("{", "")}`);
        ...
        for (const { projection, predicate } of unionProjections) {
            projectionExpr
                .when(predicate)
                .then(new cypher_builder_1.default.RawCypher((env) => `${resultVariable.getCypher(env)} ${projection.getCypher(env)}`));
        }
    }
    ...
    res.projection.push(new cypher_builder_1.default.RawCypher((env) => `${aliasVar.getCypher(env)}: ${resultVariable.getCypher(env)}`));
    return res;
}
