...
function getNodes(definitionNodes, options) {
    ...
    const nodes = definitionNodes.objectTypes.map((definition) => {
        const otherDirectives = (definition.directives || []).filter((x) => !["auth", "exclude", "node", "fulltext", "queryOptions", "plural", "shareable", "deprecated"].includes(x.name.value));
        ...
        if (resolvableArgument?.value.kind === graphql_1.Kind.BOOLEAN && resolvableArgument.value.value === false) {
            resolvable = false;
        }
